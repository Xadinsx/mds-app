import {
  ...
  DynamicQuestionType, RequirementQuestionModel, UserStoryQuestionModel, QuestionModel, createEmptyDynamicQuestion
  ...
  pageSteps.forEach((pageStep: PagesStep) => {
    ...
    pageStep.questions.forEach((question: any) => {
      ...
      if (qDynamic.type === 'Feature') {
        ...
        answersPageStep.push({ type: qDynamic.type, answer: [qFeature.description, qFeature.priority, qFeature.sprint] });
        ...
        answersPageStep.push({ type: qDynamic.type, answer: [qRequirement.description, qRequirement.feature] });
        ...
        answersPageStep.push({
          type: qDynamic.type,
          answer: [qUserStory.action, qUserStory.actor, qUserStory.goal, qUserStory.requirement]
        });
...
function addEmptyDynamicQuestion(questions: (QuestionModel | DynamicQuestionModel)[],
                                 type: DynamicQuestionType,
                                 count: number,
...
export function convertFromPageStepsCookie(pageStepsCookie: PageStepsCookieModel): { activeStepFromCookie: number, pageStepsFromCookie: PagesStep[] } {
  ...
  let lastAddedIndex: number = addEmptyDynamicQuestion(pageSteps[REQUIREMENTS_PHASE_INDEX].questions, 'Feature', pageStepsCookie.typeAnswersCount[0]);
  lastAddedIndex = addEmptyDynamicQuestion(pageSteps[REQUIREMENTS_PHASE_INDEX].questions, 'Requirement', pageStepsCookie.typeAnswersCount[1], lastAddedIndex);
  addEmptyDynamicQuestion(pageSteps[REQUIREMENTS_PHASE_INDEX].questions, 'UserStory', pageStepsCookie.typeAnswersCount[2], lastAddedIndex);
  ...
  for (let i: number = 0; i < pageSteps.length; i++) {
    for (let j: number = 0; j < pageSteps[i].questions.length; j++) {
      ...
      if (qCookie.type === 'Feature') {
        const qFeature: FeatureQuestionModel = pageSteps[i].questions[j] as FeatureQuestionModel;
        qFeature.description = qCookie.answer[0];
        qFeature.priority = qCookie.answer[1];
        qFeature.sprint = qCookie.answer[2];
      } else if (qCookie.type === 'Requirement') {
        const qRequirement: RequirementQuestionModel = pageSteps[i].questions[j] as RequirementQuestionModel;
        qRequirement.description = qCookie.answer[0];
        qRequirement.feature = qCookie.answer[1];
      } else if (qCookie.type === 'UserStory') {
        const qUserStory: UserStoryQuestionModel = pageSteps[i].questions[j] as UserStoryQuestionModel;
        ...
        pageSteps[i].questions[j].answer = pageStepsCookie.answers[i][j].answer;
      }
    }
  }
  return {
